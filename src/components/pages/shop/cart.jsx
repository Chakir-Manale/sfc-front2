import React, {useState, useEffect} from 'react';
import {Link} from 'react-router-dom';
import {connect, useDispatch} from 'react-redux';
import {Helmet} from 'react-helmet';

// import Custom Components
import PageHeader from '../../common/page-header';
import Breadcrumb from '../../common/breadcrumb';

// import {getCartTotal} from '../../../services';
// import {quantityInputs} from '../../../utils';
import {changeQty, removeFromCart, changeShipping} from '../../../actions';
import {
    removeVariantFromCartlist,
    removePackFromCartlist,
    addVariantToCartlist
} from '../../../actions/cartlistActions';
import {T} from "../../common/partials/trans";

function Cart(props) {
    const {cartlist, total, removeFromCart, prevShip} = props;

    const [shipping, setShipping] = useState(prevShip);
    const shippingPrice = {"free": 0, "standard": 10, "express": 20};

    let usercartlistvariants = props.usercartlistvariants;
    let usercartlistpacks = props.usercartlistpacks;

    let cartSellingPrice = props.cartSellingPrice;

    let user = props.user;
    const dispatch = useDispatch();

    // useEffect(() => {
    //     quantityInputs();
    // })

    // useEffect(() => {
    //     cartlist.map((item, index) => {
    //         if ( document.querySelector( `#quantity-input-wrapper-${index} .input-group input` ) ) {
    //             document.querySelector( `#quantity-input-wrapper-${index} .input-group input` ).value = item.qty;
    //         }
    //         return item;
    //     });
    // }, [cartlist]);


    // function onChangeQty( e, productId ) {
    //     dispatch(changeQty( productId, e.currentTarget.querySelector( 'input[type="number"]' ).value ));
    // }

    function onChangeShipping(val) {
        setShipping(val);
    }

    function goToCheckout() {
        props.changeShipping(shipping);
    }

    function onRefreshQty(event) {
        console.log("On Change QTY : ", event.target.value)
    }

    function onRemoveVariantFromCart(productId) {
        dispatch(removeVariantFromCartlist(productId));
    }

    function onRemovePackFromCart(productId) {
        dispatch(removePackFromCartlist(productId));
    }


    return (
        <>
            <Helmet>
                <title>Flow</title>
            </Helmet>

            <h1 className="d-none">Flow</h1>

            <div className="main">
                <PageHeader title="Shopping_Cart" subTitle=""/>
                <Breadcrumb title="Shopping_Cart" parent1={["Shop", "shop/sidebar/list"]}/>
                {
                    user.isLogged ?
                        <div className="page-content">
                            <div className="cart">
                                <div className="container">
                                    <div className="row">
                                        <div className="col-lg-9">
                                            <table className="table table-cart table-mobile">
                                                <thead>
                                                <tr>
                                                    <th><T id={"Product"}/></th>
                                                    <th><T id={"Unit.Price"}/></th>
                                                    <th><T id={"Quantity"}/></th>
                                                    <th><T id={"TOTAL"}/></th>
                                                    <th></th>
                                                </tr>
                                                </thead>

                                                <tbody>
                                                <tr><T id={"PRODUCTS"}/></tr>

                                                {
                                                    usercartlistvariants.length > 0 ?
                                                        usercartlistvariants.map((item, index) =>
                                                            <tr key={'cart-item-' + index}>
                                                                <td className="product-col">
                                                                    <div className="product">
                                                                        {/*<figure className="product-media">*/}
                                                                        {/*    <Link*/}
                                                                        {/*        to={`${process.env.PUBLIC_URL}/product/default/${item.variant.id}`}>*/}
                                                                        {/*        <img*/}
                                                                        {/*            src={`${process.env.REACT_APP_API_URL}/uploads/variant_images/` + item.variant.images[0].fileName}*/}
                                                                        {/*            alt="Product"*/}
                                                                        {/*            style={{position: "initial"}}/>*/}
                                                                        {/*    </Link>*/}
                                                                        {/*</figure>*/}

                                                                        <h3 className="product-title">
                                                                            <Link
                                                                                to={`${process.env.PUBLIC_URL}/product/default/${item.variant.id}`}>{item.variant.nameFr}</Link>
                                                                        </h3>
                                                                    </div>
                                                                </td>

                                                                <td className="price-col">
                                                                    {
                                                                        item.variant._sellingPrice
                                                                    } MAD
                                                                </td>

                                                                <td className="quantity-col"
                                                                    id={'quantity-input-wrapper-' + index}>
                                                                    {/*<div className="cart-product-quantity" onClick={ ( e ) => onChangeQty( e, item.id ) }>
                                                                <input type="number"
                                                                    className="form-control"
                                                                    defaultValue={ item.qty }
                                                                    min="1"
                                                                    max={ item.stock }
                                                                    step="1"
                                                                    data-decimals="0"
                                                                    required
                                                                />
                                                            </div>*/}
                                                                    <div
                                                                        className="details-filter-row details-row-size">
                                                                        {/*<label htmlFor="qty">*/}
                                                                        {/*    <T id={"Quantity"}/>:*/}
                                                                        {/*</label>*/}
                                                                        <div className="product-details-quantity"
                                                                             style={{marginRight: '20px'}}>
                                                                            <input type="number" id={'qty' + item.variant.id}
                                                                                   className="form-control vv"
                                                                                   defaultValue={item.qty} min="1"
                                                                                   max="100"
                                                                                   step="1" data-decimals="0" required
                                                                                   onChange={e => dispatch(addVariantToCartlist(item.variant.id, e.target.value, true))}/>
                                                                        </div>
                                                                    </div>
                                                                </td>

                                                                <td className="total-col" style={{width: '15%'}}>
                                                                    {item.qty * item.variant.price} MAD
                                                                </td>

                                                                <td className="remove-col">
                                                                    <button className="btn-remove" onClick={() => {
                                                                        onRemoveVariantFromCart(item.variant.id)
                                                                    }}>
                                                                        <i className="icon-close"/>
                                                                    </button>
                                                                    {/*<button className="btn-remove" onClick={() => {*/}
                                                                    {/*    onRefreshQty(item.variant.id)*/}
                                                                    {/*}}><i className="icon-refresh"></i></button>*/}
                                                                </td>
                                                            </tr>
                                                        ) :
                                                        <tr>
                                                            <td>
                                                                <p className="pl-2 pt-1 pb-1"><T
                                                                    id={"CART.MENU.EMPTY"}/></p>
                                                            </td>
                                                        </tr>
                                                }
                                                <tr>Packs</tr>
                                                {
                                                    usercartlistpacks.length > 0 ?
                                                        usercartlistpacks.map((item, index) =>
                                                            <tr key={'cart-item-' + index}>
                                                                <td className="product-col">
                                                                    <div className="product">
                                                                        <figure className="product-media">
                                                                            {/*<Link to={ `${process.env.PUBLIC_URL}/product/default/${item.id}` }>
                                                                        <img src={ process.env.PUBLIC_URL + '/' + item.pictures[ 0 ] } alt="Product" />
                                                                    </Link>*/}
                                                                        </figure>

                                                                        <h3 className="product-title">
                                                                            <Link
                                                                                to={`${process.env.PUBLIC_URL}/product/default/${item.id}`}>{item.pack.nameFr}</Link>
                                                                        </h3>
                                                                    </div>
                                                                </td>

                                                                <td className="price-col">
                                                                    {
                                                                        item.pack.price
                                                                    } MAD
                                                                </td>

                                                                <td className="quantity-col"
                                                                    id={'quantity-input-wrapper-' + index}>
                                                                    {/*<div className="cart-product-quantity" onClick={ ( e ) => onChangeQty( e, item.id ) }>
                                                                <input type="number"
                                                                    className="form-control"
                                                                    defaultValue={ item.qty }
                                                                    min="1"
                                                                    //max={ item.stock }
                                                                    step="1"
                                                                    data-decimals="0"
                                                                    required
                                                                />
                                                            </div>*/}
                                                                    <div
                                                                        className="details-filter-row details-row-size">
                                                                        <label htmlFor="qty"><T
                                                                            id={"Quantity"}/>:</label>
                                                                        <div className="product-details-quantity"
                                                                             style={{marginRight: '20px'}}>
                                                                            <input type="number"
                                                                                   className="form-control"
                                                                                   defaultValue={item.qty} min="1"
                                                                                   max="100"
                                                                                   step="1" data-decimals="0" required
                                                                                   onChange={e => dispatch(addVariantToCartlist(item.id, e.target.value, false))}/>
                                                                        </div>
                                                                    </div>
                                                                </td>

                                                                <td className="total-col">
                                                                    {item.qty * item.pack.price} MAD
                                                                </td>

                                                                <td className="remove-col">
                                                                    <button className="btn-remove" onClick={() => {
                                                                        onRemovePackFromCart(item.pack.id)
                                                                    }}>
                                                                        <i className="icon-close"/>
                                                                    </button>
                                                                    <button className="btn-remove" onClick={() => {
                                                                        onRefreshQty(item.pack.id)
                                                                    }}>
                                                                        <i className="icon-refresh"/>
                                                                    </button>
                                                                </td>
                                                            </tr>
                                                        ) :
                                                        <tr>
                                                            <td>
                                                                <p className="pl-2 pt-1 pb-1"><T
                                                                    id={"CART.MENU.EMPTY"}/></p>
                                                            </td>
                                                        </tr>
                                                }

                                                {/*{cartlist.length > 0 ?*/}
                                                {/*    cartlist.map((item, index) =>*/}
                                                {/*        <tr key={'cart-item-' + index}>*/}
                                                {/*            <td className="product-col">*/}
                                                {/*                <div className="product">*/}
                                                {/*                    <figure className="product-media">*/}
                                                {/*                        <Link*/}
                                                {/*                            to={`${process.env.PUBLIC_URL}/product/default/${item.id}`}>*/}
                                                {/*                            <img*/}
                                                {/*                                src={process.env.PUBLIC_URL + '/' + item.pictures[0]}*/}
                                                {/*                                alt="Product"/>*/}
                                                {/*                        </Link>*/}
                                                {/*                    </figure>*/}

                                                {/*                    <h3 className="product-title">*/}
                                                {/*                        <Link*/}
                                                {/*                            to={`${process.env.PUBLIC_URL}/product/default/${item.id}`}>{item.name}</Link>*/}
                                                {/*                    </h3>*/}
                                                {/*                </div>*/}
                                                {/*            </td>*/}

                                                {/*            <td className="price-col">*/}
                                                {/*                ${0 < item.discount ?*/}

                                                {/*                item.salePrice.toLocaleString(undefined, {*/}
                                                {/*                    minimumFractionDigits: 2,*/}
                                                {/*                    maximumFractionDigits: 2*/}
                                                {/*                }) :*/}

                                                {/*                item.price.toLocaleString(undefined, {*/}
                                                {/*                    minimumFractionDigits: 2,*/}
                                                {/*                    maximumFractionDigits: 2*/}
                                                {/*                })*/}

                                                {/*            }*/}
                                                {/*            </td>*/}

                                                {/*            <td className="quantity-col" id={'quantity-input-wrapper-' + index}>*/}
                                                {/*                <div className="cart-product-quantity"*/}
                                                {/*                     onClick={(e) => onChangeQty(e, item.id)}>*/}
                                                {/*                    <input type="number"*/}
                                                {/*                           className="form-control"*/}
                                                {/*                           defaultValue={item.qty}*/}
                                                {/*                           min="1"*/}
                                                {/*                           max={item.stock}*/}
                                                {/*                           step="1"*/}
                                                {/*                           data-decimals="0"*/}
                                                {/*                           required*/}
                                                {/*                    />*/}
                                                {/*                </div>*/}
                                                {/*            </td>*/}

                                                {/*            <td className="total-col">*/}
                                                {/*                ${item.sum.toLocaleString(undefined, {*/}
                                                {/*                minimumFractionDigits: 2,*/}
                                                {/*                maximumFractionDigits: 2*/}
                                                {/*            })}*/}
                                                {/*            </td>*/}

                                                {/*            <td className="remove-col">*/}
                                                {/*                <button className="btn-remove"*/}
                                                {/*                        onClick={(e) => removeFromCart(item.id)}><i*/}
                                                {/*                    className="icon-close"></i></button>*/}
                                                {/*            </td>*/}
                                                {/*        </tr>*/}
                                                {/*    ) :*/}
                                                {/*    <tr>*/}
                                                {/*        <td>*/}
                                                {/*            <p className="pl-2 pt-1 pb-1"> No Products in Cart </p>*/}
                                                {/*        </td>*/}
                                                {/*    </tr>*/}
                                                {/*}*/}

                                                </tbody>
                                            </table>

                                            <div className="cart-bottom">
                                                {/*<div className="cart-discount"*/}
                                                {/*     style={{minHeight: isIEBrowser() ? '40px' : 'auto'}}>*/}
                                                {/*    <form action="#">*/}
                                                {/*        <div className="input-group">*/}
                                                {/*            <input type="text" className="form-control" required=""*/}
                                                {/*                   placeholder="coupon code"/>*/}
                                                {/*            <div className="input-group-append">*/}
                                                {/*                <button className="btn btn-outline-primary-2" type="submit">*/}
                                                {/*                    <i*/}
                                                {/*                        className="icon-long-arrow-right"></i></button>*/}
                                                {/*            </div>*/}
                                                {/*        </div>*/}
                                                {/*    </form>*/}
                                                {/*</div>*/}

                                                {/*<button className="btn btn-outline-dark-2"><span>UPDATE CART</span><i*/}
                                                {/*    className="icon-refresh"></i></button>*/}
                                            </div>
                                        </div>
                                        <aside className="col-lg-3">
                                            <div className="summary summary-cart">
                                                <h3 className="summary-title">
                                                    <T id={"CART.TOTAL"}/>
                                                </h3>

                                                <table className="table table-summary">
                                                    <tbody>
                                                    <tr className="summary-subtotal">
                                                        {/*<td>Subtotal:</td>*/}
                                                        {/*<td>${total.toLocaleString(undefined, {*/}
                                                        {/*    minimumFractionDigits: 2,*/}
                                                        {/*    maximumFractionDigits: 2*/}
                                                        {/*})}</td>*/}
                                                    </tr>
                                                    <tr className="summary-shipping">
                                                        <td>
                                                            <T id={"SHIPPING"}/>:
                                                        </td>
                                                        <td>&nbsp;</td>
                                                    </tr>

                                                    <tr className="summary-shipping-row">
                                                        <td>
                                                            <div className="custom-control custom-radio">
                                                                <input type="radio"
                                                                       id="free-shipping"
                                                                       name="shipping"
                                                                       className="custom-control-input"
                                                                       onChange={(e) => onChangeShipping("free")}
                                                                       defaultChecked={"free" === prevShip ? true : false}
                                                                />
                                                                <label className="custom-control-label"
                                                                       htmlFor="free-shipping">
                                                                    <T id={"FREE.SHIPPING"}/>
                                                                </label>
                                                            </div>
                                                        </td>
                                                        <td>0.00 MAD</td>
                                                    </tr>

                                                    {/*<tr className="summary-shipping-row">*/}
                                                    {/*    <td>*/}
                                                    {/*        <div className="custom-control custom-radio">*/}
                                                    {/*            <input type="radio"*/}
                                                    {/*                   id="standard-shipping"*/}
                                                    {/*                   name="shipping"*/}
                                                    {/*                   className="custom-control-input"*/}
                                                    {/*                   onChange={(e) => onChangeShipping("standard")}*/}
                                                    {/*                   defaultChecked={"standard" === prevShip ? true : false}*/}
                                                    {/*            />*/}
                                                    {/*            <label className="custom-control-label"*/}
                                                    {/*                   htmlFor="standard-shipping">*/}
                                                    {/*                <T id={"STANDARD.SHIPPING"}/>:*/}
                                                    {/*            </label>*/}
                                                    {/*        </div>*/}
                                                    {/*    </td>*/}
                                                    {/*    <td>$10.00</td>*/}
                                                    {/*</tr>*/}

                                                    {/*<tr className="summary-shipping-row">*/}
                                                    {/*    <td>*/}
                                                    {/*        <div className="custom-control custom-radio">*/}
                                                    {/*            <input type="radio"*/}
                                                    {/*                   id="express-shipping"*/}
                                                    {/*                   name="shipping"*/}
                                                    {/*                   className="custom-control-input"*/}
                                                    {/*                   onChange={(e) => onChangeShipping("express")}*/}
                                                    {/*                   defaultChecked={"express" === prevShip ? true : false}*/}
                                                    {/*            />*/}
                                                    {/*            <label className="custom-control-label"*/}
                                                    {/*                   htmlFor="express-shipping">*/}
                                                    {/*                <T id={"EXPRESS.SHIPPING"}/>:*/}
                                                    {/*            </label>*/}
                                                    {/*        </div>*/}
                                                    {/*    </td>*/}
                                                    {/*    <td>$20.00</td>*/}
                                                    {/*</tr>*/}

                                                    {/*<tr className="summary-shipping-estimate">*/}
                                                    {/*    <td>Estimate for Your Country<br/> <a*/}
                                                    {/*        href={`${process.env.PUBLIC_URL}/shop/dashboard`}>Change*/}
                                                    {/*        address</a>*/}
                                                    {/*    </td>*/}
                                                    {/*    <td>&nbsp;</td>*/}
                                                    {/*</tr>*/}

                                                    <tr className="summary-total">
                                                        <td>Total:</td>
                                                        <td>
                                                            {cartSellingPrice} MAD
                                                        </td>
                                                        {/*<td>*/}
                                                        {/*    ${(total + shippingPrice[shipping]).toLocaleString(undefined, {*/}
                                                        {/*    minimumFractionDigits: 2,*/}
                                                        {/*    maximumFractionDigits: 2*/}
                                                        {/*})}*/}
                                                        {/*</td>*/}
                                                    </tr>
                                                    </tbody>
                                                </table>

                                                <Link
                                                    to={`/shop/checkout`}
                                                    className="btn btn-outline-primary-2 btn-order btn-block"
                                                    onClick={goToCheckout}>
                                                    <span>
                                                        <T id={"PROCEED.TO.CHECKOUT"}/>
                                                    </span>
                                                </Link>
                                                {/*<button*/}
                                                {/*    className="btn btn-outline-primary-2 btn-order btn-block"*/}
                                                {/*    onClick={goToCheckout}*/}
                                                {/*>*/}
                                                {/*    <T id={"PROCEED.TO.CHECKOUT"}/>*/}
                                                {/*</button>*/}
                                            </div>

                                            <Link to={`${process.env.PUBLIC_URL}/shop`}
                                                  className="btn btn-outline-dark-2 btn-block mb-3">
                                                <span>
                                                    <T id={"CONTINUE.SHOPPING"}/>
                                                </span>
                                                <i className="icon-refresh"/>
                                            </Link>
                                        </aside>
                                    </div>
                                </div>
                            </div>
                        </div>
                        :
                        <div className="container" style={{textAlign: 'center', marginBottom: '5%', marginTop: '5%'}}>
                            <p style={{fontSize: '55px', fontWeight: 'bold'}}>Vous devez d'abord vous connecter</p>
                        </div>
                }
            </div>
        </>
    )
}

export const mapStateToProps = (state) => ({
    usercartlistvariants: state.usercartlist.cartlist_variants ? state.usercartlist.cartlist_variants : [],
    usercartlistpacks: state.usercartlist.cartlist_packs ? state.usercartlist.cartlist_packs : [],
    user: state.user ? state.user : [],
    // cartlist: state.cartlist.cart,
    // total: getCartTotal(state.cartlist.cart),
    cartSellingPrice: state.usercartlist.sellingPrice ? state.usercartlist.sellingPrice : 0,
    prevShip: state.cartlist.shipping
})

export default connect(mapStateToProps, {
    changeQty,
    removeVariantFromCartlist,
    removePackFromCartlist,
    removeFromCart,
    changeShipping
})(Cart);